...
class BDCN_Edge_ES_Extractor(nn.Module):# Ordinary model and also the extractor of ES, output [Ordinary output, torch.cat(Ordinary output)]
    def __init__(self, pretrain=None, logger=None, rate=4):
        super(BDCN_Edge_ES_Extractor, self).__init__()
    ...
    def forward(self, x):
        ...
        Predict=[F.sigmoid(p1_1), F.sigmoid(p2_1), F.sigmoid(p3_1), F.sigmoid(p4_1), F.sigmoid(p5_1), F.sigmoid(p1_2), F.sigmoid(p2_2), F.sigmoid(p3_2), F.sigmoid(p4_2), F.sigmoid(p5_2), F.sigmoid(fuse)]
        Feature=torch.cat(Predict,dim=1)
...
class BDCN_Edge_EES_Extractor(nn.Module):#Use as an extractor in EES
    
    def __init__(self, pretrain=None, logger=None, rate=4,device='cpu'):
        super(BDCN_Edge_EES_Extractor, self).__init__()
        self.pretrain = pretrain
        t = 1
        ...
        # self.score_dsn5 = nn.Conv2d(21, 1, (1, 1), stride=1)
        # self.score_dsn5_1 = nn.Conv2d(21, 1, (1, 1), stride=1)
        self.upsample_2 = nn.ConvTranspose2d(21, 21, 4, stride=2, bias=False)
        self.upsample_4 = nn.ConvTranspose2d(21, 21, 8, stride=4, bias=False)
        self.upsample_8 = nn.ConvTranspose2d(21, 21, 16, stride=8, bias=False)
        self.upsample_8_5 = nn.ConvTranspose2d(21, 21, 16, stride=8, bias=False)
        self.fuse = nn.Conv2d(210, 1, 1, stride=1)
        self.fuse1_1 = nn.Conv2d(21, 1, 1, stride=1)
        self.fuse1_2 = nn.Conv2d(21, 1, 1, stride=1)
        self.fuse2_1 = nn.Conv2d(21, 1, 1, stride=1)
        self.fuse2_2 = nn.Conv2d(21, 1, 1, stride=1)
        self.fuse3_1 = nn.Conv2d(21, 1, 1, stride=1)
        self.fuse3_2 = nn.Conv2d(21, 1, 1, stride=1)
        self.fuse4_1 = nn.Conv2d(21, 1, 1, stride=1)
        self.fuse4_2 = nn.Conv2d(21, 1, 1, stride=1)
        self.fuse5_1 = nn.Conv2d(21, 1, 1, stride=1)
        self.fuse5_2 = nn.Conv2d(21, 1, 1, stride=1)
    ...
    def forward(self, x):
        ...
        # s2 = self.score_dsn2(sum2)
        # s21 = self.score_dsn2_1(sum2)
        s2 = self.upsample_2(sum2)
        s21 = self.upsample_2(sum2)
        # print(s2.data.shape, s21.data.shape)
        s2 = crop(s2, x, 1, 1)
        s21 = crop(s21, x, 1, 1)
        sum3 = self.conv3_1_down(self.msblock3_1(features[4])) + \
            ...
            self.conv3_3_down(self.msblock3_3(features[6]))
        # s3 = self.score_dsn3(sum3)
        s3 =self.upsample_4(sum3)
        # print(s3.data.shape)
        s3 = crop(s3, x, 2, 2)
        # s31 = self.score_dsn3_1(sum3)
        s31 =self.upsample_4(sum3)
        # print(s31.data.shape)
        s31 = crop(s31, x, 2, 2)
        sum4 = self.conv4_1_down(self.msblock4_1(features[7])) + \
            self.conv4_2_down(self.msblock4_2(features[8])) + \
            self.conv4_3_down(self.msblock4_3(features[9]))
        # s4 = self.score_dsn4(sum4)
        s4 = self.upsample_8(sum4)
        # print(s4.data.shape)
        s4 = crop(s4, x, 4, 4)
        # s41 = self.score_dsn4_1(sum4)
        s41 = self.upsample_8(sum4)
        # print(s41.data.shape)
        s41 = crop(s41, x, 4, 4)
        sum5 = self.conv5_1_down(self.msblock5_1(features[10])) + \
            self.conv5_2_down(self.msblock5_2(features[11])) + \
            self.conv5_3_down(self.msblock5_3(features[12]))
        # s5 = self.score_dsn5(sum5)
        s5 = self.upsample_8_5(sum5)
        # print(s5.data.shape)
        s5 = crop(s5, x, 0, 0)
        # s51 = self.score_dsn5_1(sum5)
        s51 = self.upsample_8_5(sum5)
        # print(s51.data.shape)
        s51 = crop(s51, x, 0, 0)
        o1, o2, o3, o4, o5 = s1.detach(), s2.detach(), s3.detach(), s4.detach(), s5.detach()
        o11, o21, o31, o41, o51 = s11.detach(), s21.detach(), s31.detach(), s41.detach(), s51.detach()
        ...
        cat=torch.cat([p1_1, p2_1, p3_1, p4_1, p5_1, p1_2, p2_2, p3_2, p4_2, p5_2], 1)
        fuse = self.fuse(cat)

        Prediction=[F.sigmoid(self.fuse1_1(p1_1)), F.sigmoid(self.fuse2_1(p2_1)), F.sigmoid(self.fuse3_1(p3_1)), F.sigmoid(self.fuse4_1(p4_1)), F.sigmoid(self.fuse5_1(p5_1)), F.sigmoid(self.fuse1_2(p1_2)), F.sigmoid(self.fuse2_2(p2_2)), F.sigmoid(self.fuse3_2(p3_2)), F.sigmoid(self.fuse4_2(p4_2)), F.sigmoid(self.fuse5_2(p5_2)), F.sigmoid(fuse)]
        Feature=torch.cat([torch.zeros(B,1,H,W).to(self.device),F.sigmoid(cat),torch.ones(B,1,H,W).to(self.device)],dim=1)
