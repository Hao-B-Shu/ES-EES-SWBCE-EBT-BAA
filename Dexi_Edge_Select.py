...
class UpConvBlock_EES(nn.Module):#输出21张
    def __init__(self, in_features, up_scale):
        super(UpConvBlock_EES, self).__init__()
        self.up_factor = 2
        self.constant_features = 21
    ...
    def make_deconv_layers(self, in_features, up_scale):
        ...
        for i in range(up_scale):
            kernel_size = 2 ** up_scale
            pad = all_pads[up_scale]  # kernel_size-1
            out_features = self.constant_features
            layers.append(nn.Conv2d(in_features, out_features, 1))
            layers.append(nn.ReLU(inplace=True))
            layers.append(nn.ConvTranspose2d(
                out_features, out_features, kernel_size, stride=2, padding=pad))
...
class Dexi_Edge_EES_Extractor(nn.Module):

    def __init__(self,device='cpu'):
        super(Dexi_Edge_EES_Extractor,self).__init__()
        ...
        self.up_block_1 = UpConvBlock_EES(64, 1)
        self.up_block_2 = UpConvBlock_EES(128, 1)
        self.up_block_3 = UpConvBlock_EES(256, 2)
        self.up_block_4 = UpConvBlock_EES(512, 3)
        self.up_block_5 = UpConvBlock_EES(512, 4)
        self.up_block_6 = UpConvBlock_EES(256, 4)

        self.block_cat = SingleConvBlock(126, 1, stride=1, use_bs=False)
        self.f1 = SingleConvBlock(21, 1, stride=1, use_bs=False)
        self.f2 = SingleConvBlock(21, 1, stride=1, use_bs=False)
        self.f3 = SingleConvBlock(21, 1, stride=1, use_bs=False)
        self.f4 = SingleConvBlock(21, 1, stride=1, use_bs=False)
        self.f5 = SingleConvBlock(21, 1, stride=1, use_bs=False)
        self.f6 = SingleConvBlock(21, 1, stride=1, use_bs=False)
    ...
    def forward(self, x):
        ...
        cat = torch.cat([out_1, out_2, out_3, out_4, out_5, out_6], dim=1)

        Prediction = [self.f1(out_1), self.f2(out_2), self.f3(out_3), self.f4(out_4), self.f5(out_5), self.f6(out_6),self.block_cat(cat)]

        Prediction=[F.sigmoid(i) for i in Prediction]
...
class Dexi_Edge_ES_Extractor(nn.Module):# Ordinary model and also the extractor of ES, output [Ordinary output, torch.cat(Ordinary output)]
    """ Definition of the DXtrem network. """

    def __init__(self):
        super(Dexi_Edge_ES_Extractor, self).__init__()
        self.block_1 = DoubleConvBlock(3, 32, 64, stride=2,)
        self.block_2 = DoubleConvBlock(64, 128, use_act=False)
    ...
    def forward(self, x):
        ...
        Feature=torch.cat(results,dim=1)
...
if __name__ == '__main__':
    ...
    model = Dexi_Edge_EES_Extractor().to(device)
    output = model(input)
    print(len(output[0]),output[1].shape)
