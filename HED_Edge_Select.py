...
class HED_Edge_EES_Extractor(nn.Module):
    def __init__(self, device):
        super(HED_Edge_EES_Extractor, self).__init__()
        # Layers.
        self.device=device
        ...
        self.score_dsn1 = nn.Conv2d(64, 21, 1)  # Out channels: 1.
        self.score_dsn2 = nn.Conv2d(128, 21, 1)
        self.score_dsn3 = nn.Conv2d(256, 21, 1)
        self.score_dsn4 = nn.Conv2d(512, 21, 1)
        self.score_dsn5 = nn.Conv2d(512, 21, 1)

        self.score_final = nn.Conv2d(105, 1, 1)
        self.score_1 = nn.Conv2d(21, 1, 1)
        self.score_2 = nn.Conv2d(21, 1, 1)
        self.score_3 = nn.Conv2d(21, 1, 1)
        self.score_4 = nn.Conv2d(21, 1, 1)
        self.score_5 = nn.Conv2d(21, 1, 1)
    ...
    def forward(self, x):
        ...
        weight_deconv2 = make_bilinear_weights(4, 21).to(self.device)
        weight_deconv3 = make_bilinear_weights(8, 21).to(self.device)
        weight_deconv4 = make_bilinear_weights(16, 21).to(self.device)
        weight_deconv5 = make_bilinear_weights(32, 21).to(self.device)

        upsample2 = torch.nn.functional.conv_transpose2d(score_dsn2, weight_deconv2, stride=2)
        upsample3 = torch.nn.functional.conv_transpose2d(score_dsn3, weight_deconv3, stride=4)
        upsample4 = torch.nn.functional.conv_transpose2d(score_dsn4, weight_deconv4, stride=8)
        upsample5 = torch.nn.functional.conv_transpose2d(score_dsn5, weight_deconv5, stride=16)
        ...
        Prediction = [self.score_1(crop1), self.score_2(crop2), self.score_3(crop3), self.score_4(crop4), self.score_5(crop5), self.score_final(fuse_cat)]
        Prediction = [torch.sigmoid(r) for r in Prediction]

        Feature = torch.cat([torch.zeros(B, 1, image_h, image_w).to(self.device), F.sigmoid(fuse_cat), torch.ones(B, 1, image_h, image_w).to(self.device)], dim=1)

        return Prediction,Feature

class HED_Edge_ES_Extractor(nn.Module):# Ordinary model and also the extractor of ES, output [Ordinary output, torch.cat(Ordinary output)]
    def __init__(self, device):
        super(HED_Edge_ES_Extractor, self).__init__()
        # Layers.
        self.device=device
...
if __name__ == '__main__':
    ...
    model = HED_Edge_EES_Extractor('cpu')
